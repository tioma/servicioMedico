...
if ($nvars > 0 && $nvars <= 50) {
	...
	$maximo = max($valores);
	$posX = 20 - strlen($maximo)*3;
	$totalValores = array_sum($valores);
	
	ImageFilledRectangle($image,0,0,$ancho,260,$colorbkg);   // crea bkg blanco
	ImageString($image,1,$posX,47,$maximo,$colores[1]);
	ImageLine($image,36,50,45,41,$colores[2]);   // diagonal
	ImageLine($image,46,41,$ancho - 32,41,$colores[3]);
	
	// 75%
	$captionGrafico = $maximo * 75 / 100;
	$posX = 20 - strlen($captionGrafico)*3;
	
	ImageString($image,1,$posX, 92,$captionGrafico,$colores[1]);
	ImageLine($image,36,95,45,86,$colores[2]);   // diagonal
	ImageLine($image,46,86,$ancho - 32,86,$colores[3]);

	// 50%
	$captionGrafico = $maximo / 2;
	$posX = 20 - strlen($captionGrafico)*3;
	
	ImageString($image,1,$posX, 137,$captionGrafico,$colores[1]);
	ImageLine($image,36,140,45,131,$colores[2]);    // diagonal
	ImageLine($image,46,131,$ancho - 32,131,$colores[3]);

	// 25%
	$captionGrafico = $maximo * 25 / 100;
	$posX = 20 - strlen($captionGrafico)*3;
	
	ImageString($image,1,$posX, 182,$captionGrafico,$colores[1]);
	...
	ImageString($image,2,31,8,$ttl,$colores[5]); // Imprime el titulo
	...
	for ($semestre = 0; $semestre < $nvars; $semestre++) {
		...
		$valorPorcentaje = round($valores[$semestre] * 100 / $totalValores);
		
		//ImageString($image,1,$equis1 + 2, 232,$semestre,$colores[0]);
		ImageString($image,1,$equis1 + 2, 51 + (180 - $porcentaje),$valorPorcentaje . '%',$colores[4]);
	...
	ImageFilledRectangle($image,0,0,300,60,$colores[4]);   // crea bkg blanco
	...
	ImageString($image,2,23,23,"Solo es posible graficar de 0 a 50 valores",$colores[5]);
}
