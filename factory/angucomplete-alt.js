...
angular.module('angucomplete-alt', [] ).directive('angucompleteAlt', ['$q', '$parse', '$http', '$sce', '$timeout', function ($q, $parse, $http, $sce, $timeout) {
	...
	return {
		...
		template:
			...
			'  <input ng-disabled="disabledInput" id="{{id}}_value" ng-model="searchStr" type="text" placeholder="{{placeholder}}" class="{{inputClass}}" ng-focus="resetHideResults()" ng-blur="hideResults()" autocapitalize="off" autocorrect="off" autocomplete="off" ng-change="inputChangeHandler(searchStr)" toupper/>' +
			'  <span class="input-group-btn"><button ng-disabled="disabledInput" class="btn btn-default" ng-click="filtrar({filtrar: filterField, contenido: \'\'}); searchStr = null"><span class="glyphicon glyphicon-remove-circle"></span></button></span>' +
			'</div>' +
